...
public class ChatClient extends Frame {
    ...
    private void initFrame(int x, int y, int w, int h) {
        ...
        this.tfText.addActionListener((actionEvent) -> {//lamda 表达式
...
//            login();
    ...
    private void logout() {
        ...
        Message message = new Message(
                MessageHeader.builder()
                        ...
                        .build(), null);
        try {
            clientChannel.write(ByteBuffer.wrap(ProtoStuffUtil.serialize(message)));//从channel往buf写
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
     ...
     * 用于接收信息的线程
     */
    private class ReceiverHandler implements Runnable {//接收网络消息，反序列化，将反序列化之后的回复内容传递给handleResponse
        ...
        private void handleResponse(Response response) {//解析协议内容
